...
public class IsmailApiStep {
    ...
    @Given("Admin enters the system with the username and password {string},{string}")
    public void adminEntersTheSystemWithTheUsernameAndPassword(String username, String password) {
        response = given().headers("Authorization","Bearer "+ReusableMethods.getIdToken(username,password),
                        "Content-Type",ContentType.JSON,
                        "Accept",ContentType.JSON)
                .when()
                .get("https://medunna.com/api/users?=size=2000");
    }
    ...
    @Given("Staff enters the system with the username and password {string},{string},{string}")
    public void staffEntersTheSystemWithTheUsernameAndPassword(String username, String password, String ssn) {
